...
class FillingTheVoidMobile < MiniTest::Test
  context "A mobile primary subtopic, in a portal" do 
    setup do 
      mobile_fire_fox_with_secure_proxy
      @proxy.new_har
      io            = File.open('test/fixtures/the_body/articles.yml')
      body_fixture  = YAML::load_documents(io)
      @body_fixture = OpenStruct.new(body_fixture[0]['filling_the_void_mobile'])
