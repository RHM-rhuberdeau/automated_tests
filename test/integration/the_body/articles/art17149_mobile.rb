...
class Art17149Mobile < MiniTest::Test
  context "A mobile Body Pro article, multiple authors" do 
    setup do 
      mobile_fire_fox_with_secure_proxy
      @proxy.new_har
      io            = File.open('test/fixtures/the_body/articles.yml')
      body_fixture  = YAML::load_documents(io)
      @body_fixture = OpenStruct.new(body_fixture[0]['art17149_mobile'])
